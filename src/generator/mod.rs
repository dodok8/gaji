...
impl TypeGenerator {
    ...
    async fn generate_index_dts(&self, action_infos: &[ActionTypeInfo]) -> Result<()> {
        ...
        // Type imports
        content.push_str("import type { JobStep, JobDefinition, WorkflowConfig, WorkflowDefinition, Step, Permissions, Service, Container, WorkflowTrigger, ScheduleTrigger, WorkflowDispatchInput, WorkflowOn, ActionInputDefinition, ActionOutputDefinition, JavaScriptActionConfig, JavaScriptActionRuns, DockerActionConfig, DockerActionRuns } from './base';\n");
...
"#);
        ...
        // Type re-exports
        content.push_str("\nexport type { JobStep, Step, JobDefinition, Service, Container, Permissions, WorkflowTrigger, ScheduleTrigger, WorkflowDispatchInput, WorkflowOn, WorkflowConfig, WorkflowDefinition, ActionInputDefinition, ActionOutputDefinition, JavaScriptActionConfig, JavaScriptActionRuns, DockerActionConfig, DockerActionRuns } from './base';\n");
        for info in &sorted_infos {
            if info.has_outputs {
                content.push_str(&format!(
