...
const publishCrates = new Job("ubuntu-latest", {
  ...
  .addStep({
    ...
    run: [
      ...
      "BODY=$(jq -n --arg token \"$OIDC_TOKEN\" '{oidc_token: $token}')",
      "CRATES_TOKEN=$(curl -sLS https://crates.io/api/v1/trusted_publishing/tokens -X POST -H 'Content-Type: application/json' -d \"$BODY\" | jq -r '.token')",
